...
Usage::
    ...
    pixi run python pure_pursuit/scripts/evaluate.py --episodes 10 --save
"""
...
RESULTS_PATH = os.path.join(MODULE_DIR, 'RESULTS.md')
...
def write_results(params, source, diag, steps, mean, std, pickle_ok, pickle_msg, args):
    ...
    parts = [f"# Pure-pursuit baseline - results",
             f"\nGenerated by `pure_pursuit/scripts/evaluate.py` on {date.today().isoformat()}.\n"]
